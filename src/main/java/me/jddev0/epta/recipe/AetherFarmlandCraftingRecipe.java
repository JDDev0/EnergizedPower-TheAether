...
public class AetherFarmlandCraftingRecipe extends CustomRecipe {
    public AetherFarmlandCraftingRecipe(CraftingBookCategory category) {
        super(category);
    }
    ...
    public boolean matches(CraftingInput container, Level level) {
        ...
        for(int i = 0;i < container.size();i++) {
            ...
            if(!itemStack.isEmpty()) {
                if(itemStack.is(AetherBlocks.AETHER_DIRT.asItem())) {
                    dirtCount++;
    ...
    public ItemStack assemble(CraftingInput container, HolderLookup.Provider registries) {
        return new ItemStack(AetherBlocks.AETHER_FARMLAND);
    }
    ...
    public RecipeSerializer<?> getSerializer() {
        return EPTARecipes.AETHER_FARMLAND_CRAFTING_RECIPE_SERIALIZER.get();
    }
